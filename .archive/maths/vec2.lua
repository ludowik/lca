local __cos, __sin, __degrees, __radians, __atan2, __sqrt = math.cos, math.sin, math.deg, math.rad, math.atan2,
                                                            math.sqrt
...
function mt.random(w, h)
    if w and h then
        return vec2(randomInt(w), randomInt(h))
    else
        w = w or 1
        return vec2(w * (random() * 2 - 1), w * (random() * 2 - 1))
    end
end
...
function mt:__tostring()
    return ("vec2{" .. "x=" .. (round(self.x, 2) or 'nan') .. ", " .. "y=" .. (round(self.y, 2) or 'nan') .. "}")
end
...
function mt:len()
    return __sqrt(self.x ^ 2 + self.y ^ 2)
end
