...
function setup()    
    ...
    parameter.action("Spawn Player", function()
        if viewerCamera == nil then return end
        
        -- Use the current camera position as the target location to spawn the player
        local pos = vec3( viewerCamera.target:unpack() )
        pos.y = 128
        
        -- Disable the initial viewerCamera so it doesn't fight with the player one
        touches.removeHandler(viewerCamera)
        scene.camera:remove(viewerCamera)
        viewerCamera = nil
    ...
    viewerCamera = scene.camera:add(OrbitViewer, vec3(16*sizeX/2,40,16*sizeZ/2), 100, 50, 400)
    viewerCamera.rx = 45
    viewerCamera.ry = 45
    ...
    createCloud(viewerCamera.target.x, 140, viewerCamera.target.z)
end
...
function update(dt)
    ...
    -- Update the voxel coordinates using the player or viewerCamera
    ...
    elseif viewerCamera then
        scene.voxels.coordinates = viewerCamera.target
    end
end
