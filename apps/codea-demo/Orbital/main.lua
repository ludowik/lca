...
function draw()
    ...
    -- draw laser
    if cursor then
        local cur = unit(vec2(touches[cursor].x, touches[cursor].y) - pos0) *2000 + pos0
        if rocket then
...
function Rocket:forces()
    ...
    if math.sqrt(vel:dot(vel)) < 50 then
        ...
        -- Slowing rocket
        local slowDownForce = - 30 * (1 + 1/math.sqrt(vel:dot(vel))) * unit(vel)
        self.body:applyForce(slowDownForce)
