...
function Music.tick()
    sounds:cache()
    if Music.map and TableEx.size(Music.map) ~= TableEx.size(Music.tuneMap) then
        for k,v in pairs(Music.tuneMap) do
            if Music.map[k] == nil then
               ...
               -- print("loading "..k)
                Music.loadProgress = Music.loadProgress + .3 / TableEx.size(Music.tuneMap)
                return nil
            end
        end
    end
...
function Music.level(name)
    ...
    for _,p in ipairs(packs) do
        if TableEx.contains(p.levels,name) then
            pack = p.name
        end
    end
