...
function Panel:add(obj)
    TableEx.map(function(x) assert(x~=obj,"adding duplicate obj") end,self.elems)
...
function Panel:translate(dx,dy)
    ...
    TableEx.map(function(x) x:translate(dx,dy) end,self.elems)
end

function Panel:flipX()
    TableEx.map(function(x) if x.flipX then x:flipX() end end, self.elems)
end

function Panel:setTint(c)
    TableEx.map(function(x) if x.setTint then x:setTint(c) end end, self.elems)
end

-- called in every frame update
function Panel:tick()
    TableEx.map(function(x) if x.tick then x:tick() end end, self.elems)
end
