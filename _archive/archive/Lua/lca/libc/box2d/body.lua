local b2Body_index = {
    ...
    applyForce = function (self, force, worldPoint)
        if worldPoint then
            box2d.b2Body_applyForce(self,
                b2Vec2(force.x*ptmRatio, force.y*ptmRatio),
                b2Vec2(worldPoint.x*ptmRatio, worldPoint.y*ptmRatio))
        else
    ...
    applyLinearImpulse = function (self, impulse)
        box2d.b2Body_applyLinearImpulse(self,
            b2Vec2(impulse.x*ptmRatio, impulse.y*ptmRatio))
    end,
    ...
    getWorldPoint = function (self, localPoint)
        local worldPoint = box2d.b2Body_getWorldPoint(self,
            b2Vec2(localPoint.x*ptmRatio, localPoint.y*ptmRatio))
...
local b2Body_mt = ffi.metatype('b2Body', {
        __newindex = function (tbl, key, value)
            ...
            elseif key == 'position' then
                box2d.b2Body_set_position(tbl, b2Vec2(value.x*ptmRatio, value.y*ptmRatio))
            ...
            elseif key == "linearVelocity" then
                box2d.b2Body_set_linearVelocity(tbl, b2Vec2(value.x*ptmRatio, value.y*ptmRatio))
