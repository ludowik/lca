...
vector.properties.get[4] = function (self) return self.w end
...
vector.properties.set[4] = function (self, w) self.w = w end
...
function vector.add(p1, x, y, z, w)
    p1.x = p1.x + x
    p1.y = p1.y + y
    p1.z = p1.z + (z or 0)
    p1.w = p1.w + (w or 0)
...
function vector.__add(p1, p2)
    return vector(p1):add(p2.x, p2.y, p2.z, p2.w)
end

function vector.sub(p1, x, y, z, w)
    p1.x = p1.x - x
    p1.y = p1.y - y
    p1.z = p1.z - (z or 0)
    p1.w = p1.w - (w or 0)
...
function vector.__sub(p1, p2)
    return vector(p1):sub(p2.x, p2.y, p2.z, p2.w)
end
...
function vector:__eq(v)
    if self.x ~= v.x or self.y ~= v.y or self.z ~= v.z or self.w ~= v.w then
        return false
    end
    return true
end
