...
function AppFreeCell:move(from, to)    
    ...
    if i ~= nil then
        ...
        local x = to:last() and to:last().x or to.x
        local y = to:last() and to:last().y or to.y
...
function AppFreeCell:reset_focus()
    self.deskFrom.focus = false
    self.deskFrom = nil
    self.deskTo = nil
end

function AppFreeCell:touchedEnded(touch)
...
function AppFreeCell:distrib(yield)
    local p = 0
...
function Suit:canCardsMoveTo(from)
    local fromCard = from:last()
    if fromCard and self:canCardMoveTo(fromCard) then
        return from:count(),fromCard
    end
end
