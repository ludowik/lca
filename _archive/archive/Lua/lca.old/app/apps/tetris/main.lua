...
Tetris.itemSize = floor(WIDTH/15)
...
function Tetris:init()
    ...
    self.scene:add(myScene():add(self.grid, self.tetrimino))
...
function Tetris:pause()
    self.__base.pause(self)
    self.active = false
    self.ui.state:reset()
end

function Tetris:resume()
    self.__base.resume(self)
    self.ui.state:start()
end
...
function Tetris:touched(touch)
    local x, y = touch.x-app.grid.parent.absolutePosition.x, touch.y-app.grid.parent.absolutePosition.y
