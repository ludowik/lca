...
function Emitter:init(n)
    ...
    self.absolutePosition = vector(WIDTH/2, HEIGHT/2)
    self.path = Table()
...
function Emitter:setPath()
    ...
    local function rect()
        local w, h = 100, 100
        self:addPointToPath(vector(x-w, y-h))
        self:addPointToPath(vector(x-w, y+h))
        self:addPointToPath(vector(x+w, y+h))
        self:addPointToPath(vector(x+w, y-h))
        self:addPointToPath(vector(x-w, y-h))
    ...
    local function random()
        for i =1,10 do
            self:addPointToPath(vector().random())
        end
    end
