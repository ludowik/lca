...
local ft_lib
function ft.load()
    if ft.loaded then return ft_lib end
    ft.loaded = true
    
    ft.defs()
    ft_lib = require(osLibPath..'.ft.ft')

    local lib
    if osx then
        lib = 'freetype'
    else
        lib = 'C:/Users/LMILHAU/Documents/Projets/Perso/Libraries/FreeType/bin/freetype6'
    end
    ft_lib = ft_lib(cload(nil, lib))
    return ft_lib
end
