...
l_image_load(lua_State *L)
{
    ...
    lua_newtable(L);
    lua_pushunsigned(L, surf->format->BytesPerPixel);
    lua_setfield(L, -2, "BytesPerPixel");

    lua_pushunsigned(L, surf->format->Rmask);
    ...
    lua_pushunsigned(L, surf->pixels);
    lua_setfield(L, -2, "pixels");

    lua_pushunsigned(L, surf);
